--[[
  ...
  mapbattle_gst is a stripped down version of the original mapbattle.
  Changes over the original mapbattle are:
  - Different format of the server messages
  - Removed all maps except instactf  
...
local ctfmaps = table.concat({
  ...
  "eternal_valley europium face-capture flagstone forge forgotten garden hallo haste hidden infamy kopenhagen l_ctf mach2 mbt1 mbt12",
  ...
  "tortuga turbulence twinforts urban_c valhalla wdcd xenon fc5 gubo duomo"
}, " ")
...
local function get_curmaprotation ()
	...
	else return server.m_ctf and ctfmaps
	end
end
...
local function registervote (info)
	...
	else playermsg("\f7[\f0MAP\f7BAT\f3TLE\f7] Your vote was \f3not\f7 accepted into the map battle pool. Wtf did you write?", info.ci) 
	end
	info.skip = true
end
...
local function catchvotes(info) 
	...
	if tonumber(info.text) == 1 or tonumber(info.text) == 2 then 
		server.sendservmsg("\f7[\f0MAP\f7BAT\f3TLE\f7] "..mb.map1.." \f0"..mb.votes.map1.."\f7 : \f0"..mb.votes.map2.."\f7 "..mb.map2.."\f0\tvoted by "..info.ci.name)
	end
end
